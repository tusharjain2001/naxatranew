      ...
      <div className="mx-auto flex max-w-1920 flex-col gap-40 px-16 py-60 xl:gap-46 xl:px-100 xl:py-100">
        ...
        <div key={variant.id + '-tech'} className="flex animate-[fade-in_0.3s_ease-out] flex-col gap-16 xl:flex-row xl:gap-26">
          <div className="flex flex-col gap-24 border border-black/25 bg-white px-16 py-20 xl:px-32 xl:py-24">
            <h2 className="text-28 tracking-display xl:text-40 xl:leading-[52px]">Technical Sketch</h2>
            <div className={`grid items-center gap-8 xl:h-600 xl:w-1172 ${sketch.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
              {sketch.map((src) => (
                <img key={src} src={src} alt="Technical drawing" className="h-auto w-full object-contain opacity-90 xl:h-full" />
              ))}
            </div>
          </div>
          ...
          <div className="no-scrollbar grid grid-cols-3 gap-12 xl:flex xl:gap-16 xl:overflow-x-auto xl:pb-2">
            {variant.applications.map((app, i) => (
              <div key={app.label + i} className="flex w-full flex-col items-center gap-3 xl:min-h-228 xl:w-182 xl:shrink-0">
                ...
                <span className="text-14 xl:text-22 xl:leading-[43px]">{app.label}</span>
              </div>
            ))}
          </div>
        </div>
